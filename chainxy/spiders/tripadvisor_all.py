...
class tripadvisor_all(scrapy.Spider):

	name = 'tripadvisor_all'
	...
	def parse(self, response):
		...
		for post in post_list:
			...
			if '(' in name:
				...
				link = self.domain + self.validate(post.xpath('.//a[@class="photo_link "]/@href')[0])

				self.driver.get(link)

				source1 = self.driver.page_source.encode("utf8")

				tree1 = etree.HTML(source1)

				post_list1 = tree1.xpath('//div[@class="attraction_clarity_cell"]')

				pagenation1 = tree1.xpath('//div[contains(@class, "pageNumbers")]//a/@href')

				for post1 in post_list1:

					if len(hotel_list) > 50:

						break

					hotel_list.append(post1)


				if pagenation1:

					for page1 in pagenation1:

						self.driver.get(self.domain + page1)

						time.sleep(2)

						source2 = self.driver.page_source.encode("utf8")

						tree2 = etree.HTML(source2)

						post_list2 = tree2.xpath('//div[@class="attraction_clarity_cell"]')

						for post2 in post_list2:

							if len(hotel_list) > 50:

								break

							hotel_list.append(post2)
				...
				if len(hotel_list) < 50 : 
					...
					for post in post_list:
						...
						if '(' in name:
							...
							if pagenation1 and len(hotel_list) < 50:
